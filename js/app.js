...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // Garante que o botão de modo teste funcione sempre
    const btnTeste = document.querySelector('.btn-test');
    if (btnTeste) {
      btnTeste.addEventListener('click', function(e) {
        e.preventDefault();
        if (typeof loginTeste === 'function') {
          loginTeste();
        } else if (window.loginTeste) {
          window.loginTeste();
        } else {
          alert('Função de modo teste não encontrada!');
        }
      });
    }
